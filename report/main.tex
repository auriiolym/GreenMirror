...
\begin{document}
	...
	% uitleggen hoe ConnectFour en de philosopher's table de mogelijkheden laten zien van GreenMirror
	...
	-- planning:
	zondag en maandag: activity diagram en sequence diagrams
	dinsdag: class diagram en implementation details
	woensdag: interface design en validation
	donderdag: discussion en javadocs
	vrijdag: afronden: generate javadocs, .jar file, check report, add references
	
	-- Vernieuwde planning:
	zondag en maandag: activity diagram, sequence diagrams, implementation details
	dinsdag: implementation details
	woensdag: interface design, discussion
	donderdag: validation
	vrijdag: background, references, generate javadocs
	...
	- more modularity, developed by more experienced developers (example of ModelTransition class)
	
	Possible extensions:
	- everything discussed in the section about possible extensions.
	- model initializer that receives the model from other tool
	- 3D objects (with "behind" placements and a 3D visualizer "moving around" objects) (Point3D is already implemented)
	
	-x extended log
	-x cached models
	-x model initializer with visual user interface
	\end{comment}
